...
check_toolchain_ssp = \
	__CROSS_CC=$(strip $1) ; \
	__HAS_SSP=`echo 'int main(){}' | $${__CROSS_CC}  -fstack-protector -x c - -o $(BUILD_DIR)/.br-toolchain-test.tmp >/dev/null 2>&1 && echo y` ; \
	...
	if [ -n "$${__SSP_OPTION}" ] ; then \
		if ! echo 'int main(){}' | $${__CROSS_CC}  $${__SSP_OPTION} -x c - -o $(BUILD_DIR)/.br-toolchain-test.tmp >/dev/null 2>&1 ; then \
